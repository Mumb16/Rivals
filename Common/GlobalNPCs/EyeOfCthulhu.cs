	...
	public class EyeOfCthulhu : GlobalNPC
	{
       ...
       public override void AI(NPC npc)
       {
			...
			if (Main.netMode != NetmodeID.MultiplayerClient && Main.masterMode)
			{
				...
				if (attackCounter <= 0 && npc.life <= 3016)
				{
					...
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						...
						{
							Projectile.NewProjectile(npc.GetSource_FromAI(), npc.Center, new Vector2(-6, 0).RotatedBy(MathHelper.ToRadians(i)), ProjectileID.BloodShot, npc.damage, 0, Main.myPlayer);
						}
					}
				...
				if (Timer > 400 && npc.life <= 1700)
				{
					...
					for (int i = 0; i < 6; i++)
					{
						...
						int projectile = Projectile.NewProjectile(npc.GetSource_FromAI(), npc.Center, direction * 5, type, damage, 10f, Main.myPlayer);
